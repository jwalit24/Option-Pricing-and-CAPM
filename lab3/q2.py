...
def loopback_without_markov(S0, T, M, r, sigma, display):
  if display == 1: 
    print("\n\n*********  Executing for M = {}  *********\n".format(M))
  curr_time = time.time()
  t = T/M
  ...
  if result:
    if display == 1:
      print("Arbitrage Opportunity exists for  {}".format(M))
    return 0, 0
  else:
    if display == 1:
      print("No arbitrage exists for {}".format(M))
...
def main():
  ...
  prices = []
  for m in M:
    prices.append(loopback_without_markov(S0 = 100, T = 1, M = m, r = 0.08, sigma = 0.20, display = 1))
  ...
  prices.clear()
  for m in M:
    prices.append(loopback_without_markov(S0 = 100, T = 1, M = m, r = 0.08, sigma = 0.20, display = 0))
  ...
  # sub-part (c)
  print("\n\n-----------------------  sub-part(c)  -----------------------")
  loopback_without_markov(S0 = 100, T = 1, M = 5, r = 0.08, sigma = 0.20, display = 2)
